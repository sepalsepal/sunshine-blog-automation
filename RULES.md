...
## 17. 클린 이미지 관리 규칙 🔒 (v1.0)

### 17.1 폴더 구조

```
contents/99_CleanReady/     ← PD님 클린 이미지 투입 폴더
contents/{콘텐츠}/0_clean/  ← 각 콘텐츠별 클린 이미지 보관
```

### 17.2 워크플로우

```
1. PD님이 99_CleanReady/에 클린 이미지 투입
2. 최부장이 이미지 확인 후 콘텐츠 매칭
3. 해당 콘텐츠 0_clean/ 폴더에 복사
...
| ✅ 복사 (cp) | 허용 |
| ❌ 이동 (mv) | 금지 |
| ❌ 삭제 | 99_CleanReady 원본 절대 삭제 금지 |

```
99_CleanReady = 원본 백업 (PD 승인 없이 삭제 불가)
```
...
| 파일명 | 대상 폴더 |
|--------|----------|
| carrot_xxx.png | 002_carrot/0_clean/ |
| apple_xxx.png | 003_apple/0_clean/ |
| blueberry_xxx.png | 004_blueberry/0_clean/ |
...
| 조건 | 노션 "표지_Clean" |
|------|------------------|
| 0_clean/ 폴더에 이미지 존재 | ✅ (체크) |
| 0_clean/ 폴더 비어있거나 없음 | 빈값 |
