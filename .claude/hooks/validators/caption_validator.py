...
def validate_caption(file_path: str) -> tuple[bool, list[str]]:
    ...
    # 보호자 경험 공유 메시지
    guardian_patterns = [
        r'같은 고민|같은 보호자|도움이 되',
        r'처음엔|고민 많이 했어요|헷갈리더라고요',
        r'함께해도|함께 해도|년째 키우면서',
        r'우리 아이|우리 강아지|우리 햇살이',
        r'저도 처음엔|저도 걱정'
    ]
    guardian_found = any(re.search(p, content) for p in guardian_patterns)
    if guardian_found:
        passes.append("보호자 동질감 메시지 PASS")
    else:
        fails.append("보호자 동질감 FAIL: 보호자 경험 공유 메시지 필요 (§6.3)")
