...
| 콘텐츠 | `./01_contents/{번호}_{영문명}/` |
| 클린 이미지 | `./01_contents/000_CleanReady/` |
...
```
01_contents/
├── 000_CleanReady/           ← 클린 이미지 원본 보관
│   ├── 01_cover/             ← 표지용 클린 이미지
│   ├── 02_food/              ← 음식 사진 클린 이미지
│   └── 03_DogWithFood/       ← 강아지+음식 브릿지 이미지
├── 001_Pumpkin/              ← 콘텐츠 폴더 (번호_영문명)
├── 002_Carrot/
├── ...
└── 165_xxx/
```

**⚠️ 폴더 이동 없음:** 게시 완료 후에도 콘텐츠 폴더는 이동하지 않음 (노션 상태만 업데이트)

---

## 🗄️ 클린 이미지 규칙

**원칙: 클린 이미지는 `000_CleanReady/`에 보관 후 각 콘텐츠 폴더로 복사 배포**

### 분류 기준

| 이미지 유형 | 보관 폴더 | 설명 |
|------------|----------|------|
| 표지용 | `000_CleanReady/01_cover/` | 강아지+음식 이미지 |
| 음식 사진 | `000_CleanReady/02_food/` | 음식만 클로즈업 |
| 브릿지 | `000_CleanReady/03_DogWithFood/` | 강아지+음식 (본문용) |

### 배포 규칙

| 작업 | 방법 |
|------|------|
| 클린 이미지 네이밍 | 원본 직접 리네이밍 (mv) |
| 콘텐츠 폴더 배포 | 000_CleanReady에서 복사 (cp) |

**⚠️ 원본 보존:** 000_CleanReady 파일은 PD 승인 2회 없이 삭제 금지
