#!/usr/bin/env python3
"""
Pillow 기반 텍스트 오버레이 모듈 v3.2

v3.1 → v3.2 주요 변경:
- 플랫폼별 오버레이 분기 추가 (WO-2026-0216-OVERLAY-R1)
  - 인스타: 01 표지만 오버레이, 02~09 원본 유지
  - 블로그: 전체 슬라이드 오버레이 적용
- render_slide_platform() 함수 추가
...
# ============================================
# 플랫폼별 오버레이 분기 (v3.2 추가)
# WO-2026-0216-OVERLAY-R1
# ============================================

def render_slide_platform(
    image_path: str,
    slide_number: int,
    platform: str = "blog",
    title: str = "",
    ...
    """
    플랫폼별 텍스트 오버레이 분기 처리 (v3.2)

    Args:
        image_path: 이미지 파일 경로
        slide_number: 슬라이드 번호 (1-based: 01=표지, 02~09=본문/CTA)
        platform: 플랫폼 ("instagram" 또는 "blog")
        title: 제목 텍스트
        subtitle: 부제목 텍스트
        safety: 안전도 ("safe", "caution", "danger", "forbidden")
        slide_type: 슬라이드 유형 ("cover", "body", "cta")

    Returns:
        처리된 이미지 경로 (원본 또는 오버레이 적용)

    플랫폼별 규칙:
        - instagram: 01 표지만 오버레이, 02~09 원본 유지
        - blog: 전체 슬라이드 오버레이 적용
    """
    # 인스타그램 02~09 슬라이드: 오버레이 미적용
    if platform.lower() == "instagram" and slide_number >= 2:
        print(f"  [SKIP v3.2] Instagram slide {slide_number:02d} - 원본 유지 (오버레이 미적용)")
        return image_path

    # 블로그 또는 인스타 01 표지: 오버레이 적용
    try:
        ...
        if slide_type == "cover" or slide_number == 1:
            ...
            result = render_cta(img, title, subtitle)
        else:
        ...
        print(f"  [OK v3.2] {platform} slide {slide_number:02d} - 오버레이 적용")
        return output_path

    except Exception as e:
        print(f"  [ERROR v3.2] render_slide_platform: {e}")
        return image_path


def should_apply_overlay(platform: str, slide_number: int) -> bool:
    """
    오버레이 적용 여부 판단 (v3.2)

    Args:
        platform: 플랫폼 ("instagram" 또는 "blog")
        slide_number: 슬라이드 번호 (1-based)
    ...
    if platform.lower() == "instagram":
        return slide_number == 1  # 01 표지만 오버레이
    return True  # 블로그는 전체 적용
...
if __name__ == "__main__":
    ...
    for platform, slide_num in test_cases:
        apply = should_apply_overlay(platform, slide_num)
        status = "적용" if apply else "미적용"
        print(f"  {platform:10} slide {slide_num:02d}: {status}")
